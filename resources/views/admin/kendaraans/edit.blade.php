    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.kendaraans.update", [$kendaraan->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('merk') ? 'is-invalid' : '' }}" type="text" name="merk" id="merk" value="{{ old('merk', $kendaraan->merk) }}" required>
                @if($errors->has('merk'))
                    <div class="invalid-feedback">
                        {{ $errors->first('merk') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('drivers') ? 'is-invalid' : '' }}" name="drivers[]" id="drivers" multiple>
                    @foreach($drivers as $id => $driver)
                        <option value="{{ $id }}" {{ (in_array($id, old('drivers', [])) || $kendaraan->drivers->contains($id)) ? 'selected' : '' }}>{{ $driver }}</option>
                    @endforeach
                </select>
